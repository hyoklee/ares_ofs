...
if [[ -z "${SLURM_NODELIST}" ]]; then
  ...
  salloc -N 2 --partition compute -w ares-comp-[29,30] --exclusive
else
  echo "${SLURM_NODELIST}"
fi
...
START=`squeue | grep storage | grep hyoklee |  cut -d ']' -f 1 | cut -d '[' -f 2 | cut -d '-' -f 1`
echo $START
STOP=`squeue | grep storage | grep hyoklee | cut -d ']' -f 1 | cut -d '[' -f 2 | cut -d '-' -f 2`
echo $STOP
rm $NND/server_nodes
...
cp $NND/server_nodes $PFS_SCRIPTS_DIR/bb_server_nodes
